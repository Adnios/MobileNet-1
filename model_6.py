    ...
    def __init_network(self):
        with tf.variable_scope('mobilenet_encoder'):
            ...
            with tf.variable_scope('conv0') as scope:
                ...
                conv = tf.nn.conv2d(preprocessed_input, weights, strides=[1, 1, 1, 1], padding='VALID')
            ...
            with tf.variable_scope('pooling0_lrn') as scope:
                pool0 = tf.nn.max_pool(conv0, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                       padding='VALID', name='pooling0')

            # print("Conv0 = ", end_time - begin_time)
            # Conv1 3*3 1
            ...
            with tf.variable_scope('conv1') as scope:
                ...
                biases = tf.get_variable('biases',
                                         ...
                                         initializer=tf.constant_initializer(0.1))
                # 卷积层 strides = [1, x_movement, y_movement, 1], padding填充周围有valid和same可选择
                conv = tf.nn.conv2d(pool0, weights, strides=[1, 1, 1, 1], padding='VALID')
                pre_activation = tf.nn.bias_add(conv, biases)  # 加入偏差
            ...
            with tf.variable_scope('conv2') as scope:
                weights = tf.get_variable('weights',
                                          shape=[3, 3, 32, 64],  # 这里只有第三位数字需要等于上一层的tensor维度
                                          dtype=tf.float32,
            ...
            # print("Conv2 = ", end_time - begin_time)

            # Conv3 3*3 1

            with tf.variable_scope('conv3') as scope:
                weights = tf.get_variable('weights',
                                          shape=[3, 3, 64, 128],  # 这里只有第三位数字96需要等于上一层的tensor维度
                                          dtype=tf.float32,
                                          initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32))
                biases = tf.get_variable('biases',
                                         shape=[128],
            ...
            # print("Conv3 = ", end_time - begin_time)

            # Conv4 3*3 3

            with tf.variable_scope('conv4') as scope:
                weights = tf.get_variable('weights',
                                          shape=[3, 3, 128, 256],  # 这里只有第三位数字96需要等于上一层的tensor维度
                                          dtype=tf.float32,
                                          initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32))
                biases = tf.get_variable('biases',
                                         shape=[256],
            ...
            with tf.variable_scope('pooling4_lrn') as scope:
                pool4 = tf.nn.max_pool(conv4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                       padding='VALID', name='pooling4')


            # print("Conv4 = ", end_time - begin_time)

            # Conv5 1*1 3

            with tf.variable_scope('conv5') as scope:
                weights = tf.get_variable('weights',
                                          shape=[1, 1, 256, 256],  # 这里只有第三位数字96需要等于上一层的tensor维度
                                          dtype=tf.float32,
                                          initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32))
                biases = tf.get_variable('biases',
                                         shape=[256],
            ...
            ############################################################################################
            self.logits = flatten(conv2d('fc', pool5, kernel_size=(1, 1), num_filters=self.args.num_classes,
