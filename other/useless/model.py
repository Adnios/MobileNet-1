...
class MobileNet:
    ...
    def __init_network(self):
        with tf.variable_scope('mobilenet_encoder'):
            ...
            # Model is here! 
            # conv1 and maxpool1 
            conv1_1 = conv2d('conv_1', preprocessed_input, num_filters=int(round(16 * self.args.width_multiplier)),
                             kernel_size=(3, 3),
                             padding='VALID', stride=(1, 1), activation=tf.nn.relu6,
                             batchnorm_enabled=self.args.batchnorm_enabled,
                             is_training=self.is_training, l2_strength=self.args.l2_strength, bias=self.args.bias,max_pool_enabled=True)
            ...
            conv2_1_dw, conv2_1_pw = depthwise_separable_conv2d('conv_ds_2', conv1_1,
                                                                ...
                                                                num_filters=32, kernel_size=(3, 3), padding='VALID',
                                                                stride=(1, 1),
                                                                batchnorm_enabled=self.args.batchnorm_enabled,
            ...
            ############################################################################################
            self.logits = flatten(conv2d('fc', conv4_1_pw, kernel_size=(1, 1), num_filters=self.args.num_classes,
                                         l2_strength=self.args.l2_strength,
                                         bias=self.args.bias))
